...
class WPFS_List_Table {
	...
	public function ajax_user_can() {
		die( 'function WPFS_List_Table::ajax_user_can() must be over-ridden in a sub-class.' );
	}

	/**
	 * Prepares the list of items for displaying.
	 * @uses WPFS_List_Table::set_pagination_args()
	 *
	 * @since 3.1.0
	 * @abstract
	 */
	public function prepare_items() {
		die( 'function WPFS_List_Table::prepare_items() must be over-ridden in a sub-class.' );
	}
	...
	public function no_items() {
		_e( 'No items found.', 'wp-full-stripe-free' );
	}
...
<?php
	...
	protected function bulk_actions( $which = '' ) {
		...
		echo '<label for="bulk-action-selector-' . esc_attr( $which ) . '" class="screen-reader-text">' . __( 'Select bulk action', 'wp-full-stripe-free' ) . '</label>';
		echo '<select name="action' . $two . '" id="bulk-action-selector-' . esc_attr( $which ) . "\">\n";
		echo '<option value="-1">' . __( 'Bulk Actions', 'wp-full-stripe-free' ) . "</option>\n";
	...
	protected function row_actions( $actions, $always_visible = false ) {
		...
		$out .= '<button type="button" class="toggle-row"><span class="screen-reader-text">' . __( 'Show more details', 'wp-full-stripe-free' ) . '</span></button>';
...
?>
		<label for="filter-by-date" class="screen-reader-text"><?php _e( 'Filter by date', 'wp-full-stripe-free' ); ?></label>
		<select name="m" id="filter-by-date">
			<option<?php selected( $m, 0 ); ?> value="0"><?php _e( 'All dates', 'wp-full-stripe-free' ); ?></option>
<?php
		foreach ( $months as $arc_row ) {
			...
			printf( "<option %s value='%s'>%s</option>\n",
				...
				sprintf( __( '%1$s %2$d', 'wp-full-stripe-free' ), $wp_locale->get_month( $month ), $year )
			);
		}
?>
		</select>
...
<?php
	...
	protected function comments_bubble( $post_id, $pending_comments ) {
		...
		if ( ! $approved_comments && ! $pending_comments ) {
			printf( '<span aria-hidden="true">&#8212;</span><span class="screen-reader-text">%s</span>',
				__( 'No comments', 'wp-full-stripe-free' )
			);
		// Approved comments have different display depending on some conditions.
			...
			printf( '<span class="post-com-count post-com-count-no-comments"><span class="comment-count comment-count-no-comments" aria-hidden="true">%s</span><span class="screen-reader-text">%s</span></span>',
				$approved_comments_number,
				$pending_comments ? __( 'No approved comments', 'wp-full-stripe-free' ) : __( 'No comments', 'wp-full-stripe-free' )
			);
		}
		...
		if ( $pending_comments ) {
			...
			printf( '<span class="post-com-count post-com-count-pending post-com-count-no-pending"><span class="comment-count comment-count-no-pending" aria-hidden="true">%s</span><span class="screen-reader-text">%s</span></span>',
				$pending_comments_number,
				$approved_comments ? __( 'No pending comments', 'wp-full-stripe-free' ) : __( 'No comments', 'wp-full-stripe-free' )
			);
		}
	}
	...
	protected function pagination( $which ) {
		...
		if ( 'bottom' === $which ) {
			...
			$total_pages_before = '<span class="screen-reader-text">' . __( 'Current Page', 'wp-full-stripe-free' ) . '</span><span id="table-paging" class="paging-input"><span class="tablenav-paging-text">';
		} else {
			$html_current_page = sprintf( "%s<input class='current-page' id='current-page-selector' type='text' name='paged' value='%s' size='%d' aria-describedby='table-paging' /><span class='tablenav-paging-text'>",
				'<label for="current-page-selector" class="screen-reader-text">' . __( 'Current Page', 'wp-full-stripe-free' ) . '</label>',
				$current,
				strlen( $total_pages )
			);
		}
	...
	public function get_columns() {
		die( 'function WPFS_List_Table::get_columns() must be over-ridden in a sub-class.' );
	}
	...
	/**
	 * Public wrapper for WPFS_List_Table::get_default_primary_column_name().
	 *
	 * @since 4.4.0
	 *
	...
	protected function get_primary_column_name() {
		...
		if ( ! isset( $columns[ $default ] ) ) {
			$default = WPFS_List_Table::get_default_primary_column_name();
		}
	...
	public function print_column_headers( $with_id = true ) {
		...
		if ( ! empty( $columns['cb'] ) ) {
			static $cb_counter = 1;
			$columns['cb'] = '<label class="screen-reader-text" for="cb-select-all-' . $cb_counter . '">' . __( 'Select All', 'wp-full-stripe-free' ) . '</label>'
...
<?php
	...
	/**
	 * Get a list of CSS classes for the WPFS_List_Table table tag.
	 *
	 * @since 3.1.0
	 *
...
<?php
	...
	protected function handle_row_actions( $item, $column_name, $primary ) {
		return $column_name === $primary ? '<button type="button" class="toggle-row"><span class="screen-reader-text">' . __( 'Show more details', 'wp-full-stripe-free' ) . '</span></button>' : '';
 	}
