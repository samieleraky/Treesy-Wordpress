...
use StripeWPFS\Stripe\StripeObject;
...
abstract class Util
{
            ...
            if (isset($resp['object']) && \is_string($resp['object'])
                ...
                if ('v2' === $apiMode && ('v2.core.event' === $resp['object'])) {
                    ...
                    if (\array_key_exists('type', $resp) && \array_key_exists($resp['type'], $eventTypes)) {
                        ...
                        $class = \StripeWPFS\Stripe\V2\Event::class;
                    }
                }
                ...
                $class = \StripeWPFS\Stripe\V2\Collection::class;
            } else {
        ...
        foreach ($properties as $key => $property) {
            if (\array_key_exists($property->getName(), $json)) {
                if ('related_object' === $property->getName()) {
                    $related_object = new \StripeWPFS\Stripe\RelatedObject();
                    ...
                    $reason = new \StripeWPFS\Stripe\Reason();
    ...
    {
        if ($h instanceof \StripeWPFS\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
