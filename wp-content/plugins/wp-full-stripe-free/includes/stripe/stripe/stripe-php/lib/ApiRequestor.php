...
class ApiRequestor
{
    ...
    private static function _encodeObjects($d)
    {
        if ($d instanceof ApiResource) {
            return \StripeWPFS\Stripe\Util\Util::utf8($d->id);
        }
        if (true === $d) {
            return 'true';
        }
        ...
        return \StripeWPFS\Stripe\Util\Util::utf8($d);
    }
    /**
    ...
    private static function _specificV1APIError($rbody, $rcode, $rheaders, $resp, $errorData)
    {
        ...
        switch ($rcode) {
            case 400:
                ...
                if ('rate_limit' === $code) {
                    return \StripeWPFS\Stripe\Exception\RateLimitException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code, $param);
                }
                if ('idempotency_error' === $type) {
                    return \StripeWPFS\Stripe\Exception\IdempotencyException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code);
                }
            // fall through in generic 400 or 404, returns InvalidRequestException by default
            // no break
            case 404:
                return \StripeWPFS\Stripe\Exception\InvalidRequestException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code, $param);
            case 401:
                return \StripeWPFS\Stripe\Exception\AuthenticationException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code);
            case 402:
                return \StripeWPFS\Stripe\Exception\CardException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code, $declineCode, $param);
            case 403:
                return \StripeWPFS\Stripe\Exception\PermissionException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code);
            case 429:
                return \StripeWPFS\Stripe\Exception\RateLimitException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code, $param);
            default:
                return \StripeWPFS\Stripe\Exception\UnknownApiErrorException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code);
        }
    }
    /**
     * @static
    ...
    private static function _specificV2APIError($rbody, $rcode, $rheaders, $resp, $errorData)
    {
        ...
        switch ($type) {
            ...
            case 'temporary_session_expired':
                return \StripeWPFS\Stripe\Exception\TemporarySessionExpiredException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code);
    ...
    private static function _specificOAuthError($rbody, $rcode, $rheaders, $resp, $errorCode)
    {
        ...
        switch ($errorCode) {
            case 'invalid_client':
                return \StripeWPFS\Stripe\Exception\OAuth\InvalidClientException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
            case 'invalid_grant':
                return \StripeWPFS\Stripe\Exception\OAuth\InvalidGrantException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
            case 'invalid_request':
                return \StripeWPFS\Stripe\Exception\OAuth\InvalidRequestException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
            case 'invalid_scope':
                return \StripeWPFS\Stripe\Exception\OAuth\InvalidScopeException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
            case 'unsupported_grant_type':
                return \StripeWPFS\Stripe\Exception\OAuth\UnsupportedGrantTypeException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
            case 'unsupported_response_type':
                return \StripeWPFS\Stripe\Exception\OAuth\UnsupportedResponseTypeException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
            default:
                return \StripeWPFS\Stripe\Exception\OAuth\UnknownOAuthErrorException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
        }
    }
    /**
     * @static
    ...
    private function _requestRaw($method, $url, $params, $headers, $apiMode, $usage, $maxNetworkRetries)
    {
        ...
        $requestStartMs = \StripeWPFS\Stripe\Util\Util::currentTimeMillis();
        ...
        if (isset($rheaders['request-id']) && \is_string($rheaders['request-id']) && '' !== $rheaders['request-id']) {
            self::$requestTelemetry = new RequestTelemetry($rheaders['request-id'], \StripeWPFS\Stripe\Util\Util::currentTimeMillis() - $requestStartMs, $usage);
        }
    ...
    private function _requestRawStreaming($method, $url, $params, $headers, $apiMode, $usage, $readBodyChunkCallable, $maxNetworkRetries)
    {
        ...
        if (isset($rheaders['request-id']) && \is_string($rheaders['request-id']) && '' !== $rheaders['request-id']) {
            self::$requestTelemetry = new RequestTelemetry($rheaders['request-id'], \StripeWPFS\Stripe\Util\Util::currentTimeMillis() - $requestStartMs);
        }
        return [$rbody, $rcode, $rheaders, $myApiKey];
    }
    /**
    ...
    {
        if (!self::$_httpClient) {
            self::$_httpClient = \StripeWPFS\Stripe\HttpClient\CurlClient::instance();
        }
    ...
    public static function streamingHttpClient()
    {
        if (!self::$_streamingHttpClient) {
            self::$_streamingHttpClient = \StripeWPFS\Stripe\HttpClient\CurlClient::instance();
        }
